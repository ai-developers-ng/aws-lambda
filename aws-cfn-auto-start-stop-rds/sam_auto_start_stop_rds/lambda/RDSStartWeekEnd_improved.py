"""
Improved RDS Start WeekEnd Lambda Function

This function starts RDS instances and clusters on weekends based on the 'StartWeekEnd' tag.
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda handler for starting RDS resources on weekends based on schedule.
    ...
    logger.info("Starting RDS Start WeekEnd function")
    ...
    try:
        ...
        for resource in all_resources:
            try:
                ...
                # Check for StartWeekEnd tag
                start_time = tag_processor.get_time_tag_value(resource.tags, 'StartWeekEnd')
                
                if start_time is None:
                    logger.debug(f"StartWeekEnd tag not found or invalid for {resource.identifier}")
                    skipped_resources.append({
                        'identifier': resource.identifier,
                        'reason': 'StartWeekEnd tag not set or invalid format'
                    })
                    continue
        ...
        message = f"WeekEnd start completed. Started: {len(started_resources)}, Skipped: {len(skipped_resources)}, Errors: {len(error_resources)}"
...
if __name__ == "__main__":
    ...
    class MockContext:
        def __init__(self):
            self.function_name = "RDSStartWeekEnd"
            self.function_version = "$LATEST"
            self.invoked_function_arn = "arn:aws:lambda:us-east-1:123456789012:function:RDSStartWeekEnd"
