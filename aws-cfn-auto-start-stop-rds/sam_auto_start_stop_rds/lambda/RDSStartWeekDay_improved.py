"""
Improved RDS Start WeekDay Lambda Function

This function starts RDS instances and clusters on weekdays based on the 'StartWeekDay' tag.
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda handler for starting RDS resources on weekdays based on schedule.
    ...
    logger.info("Starting RDS Start WeekDay function")
    ...
    try:
        ...
        if not tz_manager.is_weekday():
            message = "Today is not a weekday, skipping execution"
            logger.info(message)
            return create_lambda_response(True, message, {'is_weekday': False})
        ...
        for resource in all_resources:
            try:
                ...
                # Check for StartWeekDay tag
                start_time = tag_processor.get_time_tag_value(resource.tags, 'StartWeekDay')
                
                if start_time is None:
                    logger.debug(f"StartWeekDay tag not found or invalid for {resource.identifier}")
                    skipped_resources.append({
                        'identifier': resource.identifier,
                        'reason': 'StartWeekDay tag not set or invalid format'
                    })
                    continue
                
                # Check if current time matches the scheduled time
                if not tz_manager.is_time_in_range(start_time):
                    logger.debug(f"Not time to start {resource.identifier} (scheduled: {start_time})")
                    skipped_resources.append({
                        'identifier': resource.identifier,
                        'reason': f'Not scheduled time (scheduled: {start_time})'
                    })
                    continue
                
                logger.info(f"Time to start {resource.identifier} (scheduled: {start_time})")
        ...
        message = f"WeekDay start completed. Started: {len(started_resources)}, Skipped: {len(skipped_resources)}, Errors: {len(error_resources)}"
...
if __name__ == "__main__":
    ...
    class MockContext:
        def __init__(self):
            self.function_name = "RDSStartWeekDay"
            self.function_version = "$LATEST"
            self.invoked_function_arn = "arn:aws:lambda:us-east-1:123456789012:function:RDSStartWeekDay"
