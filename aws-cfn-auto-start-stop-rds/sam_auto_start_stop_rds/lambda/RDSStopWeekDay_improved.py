"""
Improved RDS Stop WeekDay Lambda Function

This function stops RDS instances and clusters on weekdays based on the 'StopWeekDay' tag.
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda handler for stopping RDS resources on weekdays based on schedule.
    ...
    logger.info("Starting RDS Stop WeekDay function")
    ...
    try:
        ...
        for resource in all_resources:
            try:
                ...
                # Check for StopWeekDay tag
                stop_time = tag_processor.get_time_tag_value(resource.tags, 'StopWeekDay')
                
                if stop_time is None:
                    logger.debug(f"StopWeekDay tag not found or invalid for {resource.identifier}")
                    skipped_resources.append({
                        'identifier': resource.identifier,
                        'reason': 'StopWeekDay tag not set or invalid format'
                    })
                    continue
        ...
        message = f"WeekDay stop completed. Stopped: {len(stopped_resources)}, Skipped: {len(skipped_resources)}, Errors: {len(error_resources)}"
...
if __name__ == "__main__":
    ...
    class MockContext:
        def __init__(self):
            self.function_name = "RDSStopWeekDay"
            self.function_version = "$LATEST"
            self.invoked_function_arn = "arn:aws:lambda:us-east-1:123456789012:function:RDSStopWeekDay"
